...
const GameDetail = () => {
  ...
  const handleSessionSubmit = async (e) => {
    ...
    try {
      ...
      // Use the shared `api` helper which already attaches the Authorization header
      // It uses the environment VITE_API_URL (fallback to https://api.viksorasports.com/api)
      const gameId = game._id;
      console.log('Using game ID for booking:', gameId);

      // The `api` helper expects endpoints relative to the baseURL (which already contains '/api')
      const endpoint = `/games/${gameId}/session`;
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }} style={{ maxWidth: 'none', width: '95%' }}>
      ...
      {bookingSuccess && (
        <Alert severity="success" sx={{ mb: 3, animation: `${fadeIn} 0.5s ease-out` }}>
